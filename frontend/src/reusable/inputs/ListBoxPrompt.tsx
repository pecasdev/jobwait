...
export default function ListBoxPrompt(props: PromptTypeProps) {
    ...
    return (
        ...
        >
            <ListboxButton
                className={`min-h-10 relative block w-auto min-w-32 rounded-lg bg-black/5 py-1.5 pr-8 pl-3 text-left text-sm/6 text-black
                    focus:outline-none data-[focus]:outline-2 data-[focus]:-outline-offset-2 data-[focus]:outline-black/25`}
            >
                {currentSelected}
                <ChevronDownIcon
                    className="pointer-events-none absolute top-4 right-2.5 size-4 fill-black/60"
                    aria-hidden="true"
                />
            </ListboxButton>
            <Transition
                    ...
                    {props.choices?.map((choice) => (
                        ...
                        >
                            {({ focus, selected }) => (
                                <div
                                    className={clsx(
                                        "flex cursor-default items-center gap-2 rounded-lg py-1.5 px-3 select-none data-[focus]:bg-black/10",
